...
func (p *Peer) startProtocols(writeStart <-chan struct{}, writeErr chan<- error) {
	...
	for _, proto := range p.running {
		...
		p.log.Trace(fmt.Sprintf("[Peer]Starting protocol %s/%d", proto.Name, proto.Version))
		go func() {
			defer p.wg.Done()
			err := proto.Run(p, rw)
			if err == nil {
				p.log.Trace(fmt.Sprintf("[Peer]Protocol %s/%d returned", proto.Name, proto.Version))
				err = errProtocolReturned
			} else if !errors.Is(err, io.EOF) {
				p.log.Trace(fmt.Sprintf("[Peer]Protocol %s/%d failed", proto.Name, proto.Version), "err", err)
			}
			p.protoErr <- err
		}()
	}
