...
func (t *UDPv4) findnode(toid enode.ID, toAddrPort netip.AddrPort, target v4wire.Pubkey) ([]*enode.Node, error) {
	...
	rm := t.pending(toid, toAddrPort.Addr(), v4wire.NeighborsPacket, func(r v4wire.Packet) (matched bool, requestDone bool) {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				...
				//  add the data to the udpv4 list
				// get agent name using the node id
				udpv4s = append(udpv4s, FindNode{
					discV: "v4",
					type_: "FINDNODE",
					agent: "unknown",
					msg:   "Invalid neighbor node received",
					tid:   toid.String(),
					tAddr: toAddrPort.String(),
					tKey:  target.ID().String(),
					nid:   rn.ID.ID().String(), // Neighbor Node ID:
					nAddr: rn.IP.String() + ":" + fmt.Sprint(rn.UDP),
					nKey:  rn.ID.ID().String(),
				})
				continue
			}
			// add the data to the udpv4 list
	...
	// fmt.Println("V4.findnode[REQ]: ", REQ.Name())
	t.send(toAddrPort, toid, REQ)
...
func (t *UDPv4) nodeFromRPC(sender netip.AddrPort, rn v4wire.Node) (*enode.Node, error) {
	...
	// fmt.Printf("V4.nodeFromRPC[Node]-2: %+v\n", n.Record())
	err = n.ValidateComplete()
	return n, err
}

func nodeToRPC(n *enode.Node) v4wire.Node {
	// fmt.Printf("V4.nodeToRPC[Node]: %+v\n", n)
...
func (t *UDPv4) handleFindnode(h *packetHandlerV4, from netip.AddrPort, fromID enode.ID, mac []byte) {
	...
	// show all the nodes in the console
	// fmt.Printf("handleFindnode[Node]: %+v\n", p.Nodes)
	var sent bool
